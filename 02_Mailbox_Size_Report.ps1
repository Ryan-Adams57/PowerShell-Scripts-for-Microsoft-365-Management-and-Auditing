<#
.SYNOPSIS
    Mailbox Size Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Mailbox Size Report for Microsoft 365.
...
.EXAMPLE
    .\02_Mailbox_Size_Report.ps1
    Generates standard report.

.EXAMPLE
    .\02_Mailbox_Size_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Mailbox_Size_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Mailbox Size Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-Mailbox Size Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Mailbox Size Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "Mailbox Size Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
