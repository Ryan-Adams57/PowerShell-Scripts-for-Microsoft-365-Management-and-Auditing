<#
.SYNOPSIS
    Admin Roles Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Admin Roles Report for Microsoft 365.
...
.EXAMPLE
    .\11_Admin_Roles_Report.ps1
    Generates standard report.

.EXAMPLE
    .\11_Admin_Roles_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Admin_Roles_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Connect-M365Service {
    try {
        ...
        try {
            ...
            Connect-MgGraph -Scopes @("RoleManagement.Read.Directory", "Directory.Read.All") -UseDeviceCode -NoWelcome -ErrorAction Stop
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Admin Roles Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-Admin Roles Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Admin Roles Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "Admin Roles Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
