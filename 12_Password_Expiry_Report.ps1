<#
.SYNOPSIS
    Password Expiry Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Password Expiry Report for Microsoft 365.
...
.EXAMPLE
    .\12_Password_Expiry_Report.ps1
    Generates standard report.

.EXAMPLE
    .\12_Password_Expiry_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Password_Expiry_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Password Expiry Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-Password Expiry Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Password Expiry Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "Password Expiry Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
