<#
.SYNOPSIS
    Conditional Access Policies Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Conditional Access Policies Report for Microsoft 365.
...
.EXAMPLE
    .\13_Conditional_Access_Policies_Report.ps1
    Generates standard report.

.EXAMPLE
    .\13_Conditional_Access_Policies_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Conditional_Access_Policies_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Conditional Access Policies Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-Conditional Access Policies Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Conditional Access Policies Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "Conditional Access Policies Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
