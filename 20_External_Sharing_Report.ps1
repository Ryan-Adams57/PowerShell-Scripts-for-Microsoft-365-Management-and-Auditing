<#
.SYNOPSIS
    External Sharing Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive External Sharing Report for Microsoft 365.
...
.EXAMPLE
    .\20_External_Sharing_Report.ps1
    Generates standard report.

.EXAMPLE
    .\20_External_Sharing_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_External_Sharing_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving External Sharing Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-External Sharing Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
External Sharing Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "External Sharing Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
