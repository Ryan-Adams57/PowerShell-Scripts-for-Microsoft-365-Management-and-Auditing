<#
.SYNOPSIS
    Sign In Logs Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Sign In Logs Report for Microsoft 365.
...
.EXAMPLE
    .\22_Sign_In_Logs_Report.ps1
    Generates standard report.

.EXAMPLE
    .\22_Sign_In_Logs_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Sign_In_Logs_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Connect-M365Service {
    try {
        ...
        try {
            ...
            Connect-MgGraph -Scopes @("AuditLog.Read.All") -UseDeviceCode -NoWelcome -ErrorAction Stop
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Sign In Logs Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-Sign In Logs Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Sign In Logs Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "Sign In Logs Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
