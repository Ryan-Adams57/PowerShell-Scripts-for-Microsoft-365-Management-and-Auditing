...
if ($script:Results.Count -gt 0) {
    ...
    Write-Host "  Total Permissions: $($script:Results.Count)" -ForegroundColor White
    Write-Host "  FullAccess: $(($script:Results | Where-Object { $_.PermissionType -eq 'FullAccess' }).Count)" -ForegroundColor White
    Write-Host "  SendAs: $(($script:Results | Where-Object { $_.PermissionType -eq 'SendAs' }).Count)" -ForegroundColor White
    Write-Host "  SendOnBehalf: $(($script:Results | Where-Object { $_.PermissionType -eq 'SendOnBehalf' }).Count)" -ForegroundColor White
    ...
    $script:Results | Select-Object -First 10 | Format-Table Mailbox, User, PermissionType, AccessRights -AutoSize
