<#
.SYNOPSIS
    Stale Devices Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Stale Devices Report for Microsoft 365.
...
.EXAMPLE
    .\27_Stale_Devices_Report.ps1
    Generates standard report.

.EXAMPLE
    .\27_Stale_Devices_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Stale_Devices_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Stale Devices Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Sample-Stale Devices Report-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Stale Devices Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "Stale Devices Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
