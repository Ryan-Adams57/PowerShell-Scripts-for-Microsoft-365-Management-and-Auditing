...
if ($script:Results.Count -gt 0) {
    ...
    Write-Host "  Total Environments: $($script:Results.Count)" -ForegroundColor White
    ...
    if ($IncludeResourceCounts) {
        ...
        Write-Host "    Power Apps: $(($script:Results | Measure-Object -Property PowerAppsCount -Sum).Sum)" -ForegroundColor White
        Write-Host "    Flows: $(($script:Results | Measure-Object -Property FlowsCount -Sum).Sum)" -ForegroundColor White
        Write-Host "    Connectors: $(($script:Results | Measure-Object -Property ConnectorsCount -Sum).Sum)" -ForegroundColor White
    }
    ...
    $script:Results | Select-Object -First 10 | Format-Table EnvironmentName, Type, Region, PowerAppsCount, FlowsCount -AutoSize
