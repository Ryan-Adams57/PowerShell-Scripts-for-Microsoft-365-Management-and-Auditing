<#
====================================================================================
Script Name: Get-PowerAppsUsageReport.ps1
...
SCRIPT HIGHLIGHTS:
• Retrieves all Power Apps across all environments
• Shows app type (canvas, model-driven, portal)
• Tracks app owners, creators, and shared users
• Identifies app launch statistics and usage metrics
• Lists connector dependencies for each app
• Supports filtering by environment and app type
• Generates adoption and governance analytics
...
if ($results.Count -gt 0) {
    ...
    Write-Host "  Total Apps: $($results.Count)" -ForegroundColor White
    ...
    Write-Host "  Published Apps: $(($results | Where-Object { $_.Published -eq $true }).Count)" -ForegroundColor Green
    
    if ($IncludeSharedUsers) {
        Write-Host "  Total Shared Users: $(($results | Measure-Object -Property SharedWithCount -Sum).Sum)" -ForegroundColor White
    }
    ...
    $results | Select-Object -First 10 | Format-Table AppName, AppType, Owner, Published, SharedWithCount -AutoSize
