<#
====================================================================================
Script Name: 31-Get-PowerBIWorkspaceReport.ps1
...
if ($script:Results.Count -gt 0) {
    ...
    Write-Host "  Total Workspaces: $($script:Results.Count)" -ForegroundColor White
    ...
    if ($IncludeDatasets) {
        Write-Host "  Total Datasets: $(($script:Results | Measure-Object -Property DatasetCount -Sum).Sum)" -ForegroundColor White
    }
    if ($IncludeReports) {
        Write-Host "  Total Reports: $(($script:Results | Measure-Object -Property ReportCount -Sum).Sum)" -ForegroundColor White
    }
    ...
    $script:Results | Select-Object -First 10 | Format-Table WorkspaceName, Type, IsOrphaned, AdminCount, DatasetCount, ReportCount -AutoSize
