<#
====================================================================================
Script Name: 41-Get-AzureADIdentityProtectionReport.ps1
...
if ($script:Results.Count -gt 0) {
    ...
    if ($IncludeRiskyUsers) {
        ...
        Write-Host "    High Risk: $(($script:Results | Where-Object { $_.Type -eq 'Risky User' -and $_.RiskLevel -eq 'high' }).Count)" -ForegroundColor Red
        Write-Host "    Medium Risk: $(($script:Results | Where-Object { $_.Type -eq 'Risky User' -and $_.RiskLevel -eq 'medium' }).Count)" -ForegroundColor Yellow
        Write-Host "    Low Risk: $(($script:Results | Where-Object { $_.Type -eq 'Risky User' -and $_.RiskLevel -eq 'low' }).Count)" -ForegroundColor Green
    }
    ...
    $script:Results | Select-Object -First 10 | Format-Table Type, Name, RiskLevel, State, UserPrincipalName -AutoSize
