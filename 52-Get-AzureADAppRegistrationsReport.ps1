...
if ($script:Results.Count -gt 0) {
    ...
    Write-Host "  Apps in Report: $($script:Results.Count)" -ForegroundColor White
    ...
    if ($IncludePermissions) {
        Write-Host "  Total Permissions Tracked: $(($script:Results | Measure-Object -Property PermissionCount -Sum).Sum)" -ForegroundColor Cyan
    }
    ...
    $script:Results | Select-Object -First 10 | Format-Table ApplicationName, IsOrphaned, HasExpiringCredentials, DaysUntilExpiry -AutoSize
