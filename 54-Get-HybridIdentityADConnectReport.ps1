<#
====================================================================================
Script Name: Get-HybridIdentityADConnectReport.ps1
...
SCRIPT HIGHLIGHTS:
• Retrieves Azure AD Connect synchronization status
• Shows sync errors and conflicts
• Lists directory sync configuration details
• Identifies objects failing to sync
• Tracks last sync time and sync cycles
• Supports filtering by sync status
• Generates hybrid identity health reports
• Critical for hybrid environment management
...
if ($results.Count -gt 0) {
    ...
    Write-Host "  Total Synced Objects: $($results.Count)" -ForegroundColor White
    ...
    $results | Select-Object -First 10 | Format-Table ObjectType, DisplayName, HasSyncError, LastSyncDateTime -AutoSize
