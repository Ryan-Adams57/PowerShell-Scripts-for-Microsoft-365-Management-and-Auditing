<#
====================================================================================
Script Name: 55-Get-ComplianceManagerAssessmentsReport.ps1
...
if ($script:Results.Count -gt 0) {
    ...
    Write-Host "  Active Assessments: $(($script:Results | Where-Object { $_.Status -eq 'Active' }).Count)" -ForegroundColor Yellow
    Write-Host "  Completed Assessments: $(($script:Results | Where-Object { $_.Status -eq 'Completed' }).Count)" -ForegroundColor Green
    Write-Host "  Average Completion: $([math]::Round(($script:Results | Measure-Object -Property CompletionPercentage -Average).Average, 2))%" -ForegroundColor Cyan
    
    # Assessment health breakdown
    Write-Host "`n  Assessment Health:" -ForegroundColor Cyan
    $script:Results | Group-Object AssessmentHealth | ForEach-Object {
        $color = switch ($_.Name) {
            "Good" { "Green" }
            "Stale" { "Yellow" }
            "At Risk" { "Red" }
    ...
    $script:Results | Select-Object -First 10 | Format-Table AssessmentName, Status, CompletionPercentage, AssessmentHealth, DaysSinceModified -AutoSize
