<#
====================================================================================
Script Name: 56-Get-VivaInsightsAdoptionReport.ps1
...
if ($script:Results.Count -gt 0) {
    ...
    Write-Host "  Average Collaboration Score: $([math]::Round(($script:Results | Measure-Object -Property CollaborationScore -Average).Average, 1))" -ForegroundColor White
    Write-Host "  Average Meeting Hours: $([math]::Round(($script:Results | Measure-Object -Property MeetingHours -Average).Average, 1))" -ForegroundColor Yellow
    
    if ($IncludeWellbeingMetrics) {
        Write-Host "`n  Wellbeing Metrics:" -ForegroundColor Cyan
        Write-Host "    Average Focus Hours: $([math]::Round(($script:Results | Measure-Object -Property FocusHours -Average).Average, 1))" -ForegroundColor White
        Write-Host "    Average After-Hours Work: $([math]::Round(($script:Results | Measure-Object -Property AfterHoursWork -Average).Average, 1)) hours" -ForegroundColor Yellow
        Write-Host "    Average Wellbeing Score: $([math]::Round(($script:Results | Measure-Object -Property WellbeingScore -Average).Average, 0))/100" -ForegroundColor Green
        Write-Host "    High Burnout Risk: $(($script:Results | Where-Object { $_.BurnoutRisk -eq 'High' }).Count) users" -ForegroundColor Red
    }
    
    # Engagement distribution
    Write-Host "`n  Engagement Distribution:" -ForegroundColor Cyan
    $script:Results | Group-Object EngagementLevel | Sort-Object Name | ForEach-Object {
        $color = switch ($_.Name) {
            "High" { "Green" }
            "Medium" { "Yellow" }
            "Low" { "Red" }
    ...
    $script:Results | Sort-Object CollaborationScore -Descending | Select-Object -First $TopUsers | ForEach-Object {
    ...
    if ($DetailedAnalytics) {
        ...
        $script:Results | Select-Object -First 10 | Format-Table DisplayName, EngagementLevel, CollaborationScore, MeetingHours, WellbeingScore -AutoSize
    }
