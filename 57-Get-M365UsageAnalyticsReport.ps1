...
try {
    ...
    if ($IncludeTrendAnalysis -and $script:Results.Count -gt 1) {
        Write-Host "Calculating trend analysis..." -ForegroundColor Cyan
        
        $firstRecord = $script:Results | Select-Object -Last 1
        $latestRecord = $script:Results | Select-Object -First 1
...
if ($script:Results.Count -gt 0) {
    ...
    if ($DetailedBreakdown) {
        ...
        $script:Results | Select-Object -First 10 | Format-Table ReportDate, Office365Active, ExchangeActive, TeamsActive, SharePointActive -AutoSize
    }
