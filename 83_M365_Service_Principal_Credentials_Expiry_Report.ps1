<#
.SYNOPSIS
    M365 Service Principal Credentials Expiry Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive M365 Service Principal Credentials Expiry Report for Microsoft 365.
...
.EXAMPLE
    .\083_M365_Service_Principal_Credentials_Expiry_Report.ps1
    Generates standard report.

.EXAMPLE
    .\083_M365_Service_Principal_Credentials_Expiry_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_M365_Service_Principal_Credentials_Expiry_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Connect-M365Service {
    try {
        ...
        try {
            ...
            Connect-MgGraph -Scopes @("Application.Read.All") -UseDeviceCode -NoWelcome -ErrorAction Stop
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving M365 Service Principal Credentials Expiry Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "M365 Service Principal Credentials Expiry Report-Sample-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
M365 Service Principal Credentials Expiry Report - Summary Report
========================================
...
try {
    ...
    Write-ColorOutput "M365 Service Principal Credentials Expiry Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
