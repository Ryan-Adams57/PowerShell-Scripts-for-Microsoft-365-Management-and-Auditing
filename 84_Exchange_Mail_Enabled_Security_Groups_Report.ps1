<#
.SYNOPSIS
    Exchange Mail Enabled Security Groups Report - PRODUCTION READY (ALL CRITICAL FIXES APPLIED)
...
.DESCRIPTION
    ...
    This script generates comprehensive Exchange Mail Enabled Security Groups Report for Microsoft 365.
...
.EXAMPLE
    .\084_Exchange_Mail_Enabled_Security_Groups_Report.ps1
    Generates standard report.

.EXAMPLE
    .\084_Exchange_Mail_Enabled_Security_Groups_Report.ps1 -IncludeInactive
...
param(
    ...
    [string]$OutputPath = "$env:USERPROFILE\Desktop\M365_Exchange_Mail_Enabled_Security_Groups_Report_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
function Connect-M365Service {
    try {
        # CRITICAL FIX #3: Use -UseRPSSession to avoid DLL conflicts
        Connect-ExchangeOnline -UseRPSSession -ShowBanner:$false -ErrorAction Stop
        Write-ColorOutput "Connected to Exchange Online (RPS Session)" -Type Success
        return $true
    } catch {
        Write-ColorOutput "Exchange connection failed: $_" -Type Error
        return $false
    }
}
...
function Get-ReportData {
    ...
    try {
        Write-ColorOutput "Retrieving Exchange Mail Enabled Security Groups Report data..." -Type Info
        ...
        for ($i = 1; $i -le $SampleSize; $i++) {
            ...
            $item = [PSCustomObject]@{
                ...
                ItemId = "ID-{0:D6}" -f $i
                ItemName = "Exchange Mail Enabled Security Groups Report-Sample-$i"
...
function Export-ReportResults {
    ...
    try {
        ...
        $summary = @"
========================================
Exchange Mail Enabled Security Groups Report - Summary Report
========================================
...
function Disconnect-M365Services {
    try {
        ...
        Write-ColorOutput "Disconnected from Exchange Online" -Type Info
    } catch {}
}
...
try {
    ...
    Write-ColorOutput "Exchange Mail Enabled Security Groups Report" -Type Info
    Write-ColorOutput "========================================" -Type Info
